// ── Status Badge ────────────────────────────────────────────────────────────
// Open=red  In Progress=yellow  Pending Part=blue  Closed=green
...
export function CategoryBadge({ category }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${MAP[category] || 'bg-slate-50 text-slate-600'}`}>
      {category}
    </span>
  )
}
